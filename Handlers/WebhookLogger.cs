...
{
	internal class WebhookLogger : IUpdateHandler
	{
		private readonly ILogger<WebhookLogger> _logger;

		public WebhookLogger(ILogger<WebhookLogger> logger) { _logger = logger; }
		...
		public Task HandleAsync(IUpdateContext context, UpdateDelegate next, CancellationToken cancellationToken)
		{
			...
			_logger.LogInformation("Received update {0} in a webhook at {1}.",
										  context.Update.Id,
